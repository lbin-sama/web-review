...
事件循环:
    ...
    2. 每一次循环会检查消息队列中是否有任务存在。如果有，就取出第一个任务执行，
        执行完一个后进入下一次循环；如果没有，则进入休眠状态。
...
何为异步？
    ...
    **渲染主线程承担着极其重要的工作，无论如何都不能阻塞！**
    ...
    如何理解 JS 的异步？
        ...
        自身立即结束任务的执行，转而执行后续代码。当其他线程完成时，将事先传递的回调函数包装成任务，加入到消息队列的末尾排队，
        等待主线程调度执行。
...
### JS为何会阻碍渲染？
    ...
    点击按钮后，页面上h1的内容三秒后才会发生改变，为什么，
    其实h1.textContent 已经改变了，只是页面没有发生变化，textContent改变之后，会产生绘制任务，
    所以要排队，等delay执行完成之后，才会执行，绘制页面
...
任务没有优先级，在消息队列中先进先出
    ...
    3. 按照 W3C 的标准，浏览器实现计时器时，如果嵌套层级超过 5 层，则会带有 4 毫秒的最少时间，
